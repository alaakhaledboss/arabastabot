...
3. **الصيد وجمع المواد**
    استخدم `%hunt forest` للصيد في الغابة و`%hunt lake` للصيد في البحيرة. يجب تنفيذ الأمر داخل القناة المخصصة له فقط، وإلا سيرفضه البوت. الصيد مرتبط بـ 20 دقيقة انتظار، ويستهلك HP حسب المسار والتخصص والمحتوى المقطوع، لذلك راقب شريط HP في الملف الشخصي بعد كل محاولة.
...
12. **متى تستخدم `%man`**
    إذا نسيت صيغة أمر معيّن، استخدم `%man <command>` بدل التخمين. هذا مفيد خاصة مع الأوامر التي لها سياق خاص مثل `%hunt` و`%craft` و`%sell` و`%clan admincreate`. دليل الأمر يعرض لك الصيغة والأمثلة مباشرة من داخل البوت.
