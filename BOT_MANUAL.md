...
هذا القسم يشرح كيف يستخدم اللاعب البوت فعليًا خطوة بخطوة، وما الأمر المناسب لكل حالة، وكيف يتفاعل مع الأزرار والقوائم والنماذج داخل Discord.

1. **بدء الاستخدام وفهم الحالة العامة**
    ابدأ دائمًا بأمر `%profile` أو `%p` حتى ترى مستوى اللاعب، المسار الحالي، التخصص، HP، والعتاد المجهز. إذا كنت تريد معرفة كل الأوامر المتاحة لك، استخدم `%help` للأوامر العامة، و`%commands` إذا كنت من المصرّح لهم أو من الإدارة، و`%owner` إذا كنت المالك.

2. **اختيار المسار والتخصص**
    نظام التقدم هو نقطة البداية الحقيقية للعب. استخدم `%specialty` عندما يكون مسارك أو إعدادك التقدمي بحاجة لاختيار تخصص، ثم تأكد من أن المسار ظاهر في الملف الشخصي. الصيد والتصنيع وبعض المكافآت تعتمد على المسار والتخصص، لذلك لا تبدأ الأنظمة الأخرى قبل التأكد من هذه الحالة.

3. **الصيد وجمع المواد**
    يتم تنفيذ الصيد والاستكشاف عبر أزرار Hunt / Research داخل القناة المخصصة للغابة أو البحيرة. لا تستخدم الأمر النصي القديم، لأن النظام الجديد يعتمد على الأزرار الدائمة في القناة. الصيد مرتبط بـ 20 دقيقة انتظار، ويستهلك HP حسب المسار والتخصص والمحتوى المقطوع، لذلك راقب شريط HP في الملف الشخصي بعد كل محاولة.

4. **فهم HP والتنبيه والخطر**
    HP يبدأ من 100 ويُحفظ في قاعدة البيانات. عندما يصل إلى 10 أو أقل، يرسل البوت تنبيهًا خاصًا في الخاص. عندما يصل إلى 5 أو أقل، يمنعك البوت من الصيد أو الاستكشاف حتى تتعافى. إذا وصل إلى 0، يفعّل نظام وفاة المقاتل ويعيد ضبط التقدم القتالي المرتبط بالعقوبة.

5. **العتاد وتأثيره في اللعب**
    استخدم `%gear status` لرؤية العتاد، و`%gear inventory` لمراجعة الموجود، ثم `%gear equip` و`%gear unequip` لتبديل العناصر. العتاد ليس للعرض فقط؛ بعض القطع تقلل الضرر في الصيد، لذلك تجهيزك يؤثر مباشرة في HP الذي تخسره أثناء اللعب.

6. **التصنيع الجماعي**
    استخدم `%craft` عندما تكون على مسار Atelier. سيظهر لك لوح تفاعلي بأزرار للوصفات المتاحة. صاحب الجلسة يبدأ وصفة التصنيع، ثم يمكن لأعضاء الكلان المساهمة من موادهم الخاصة عبر الأزرار نفسها. عندما تكتمل المواد المطلوبة، يحصل صاحب الجلسة على الناتج النهائي في مخزونه.

7. **البيع والسوق**
    استخدم `%sell <item> <price>` عندما تكون على مسار Merchant. سيأخذ البوت العنصر أو المادة من مخزونك ويحوّلها إلى قائمة بيع تحتوي زر شراء. الشراء يتم من نفس الإعلان التفاعلي، ولا يسمح لأعضاء نفس الكلان بالشراء من بعضهم.

8. **الكلان والإدارة الجماعية**
    استخدم `%clan` لعرض القائمة، `%clan status` لرؤية الحالة الحالية، `%clan leave` لمغادرة الكلان، و`%clan disband` إذا كنت القائد وتريد حل الكلان. الأمر الإداري `%clan admincreate` يُستخدم فقط من الإدارة لإنشاء كلان جديد من عدة أعضاء على نفس المسار.

9. **البنك والمتجر والتحويلات**
    `%bank` و`%shop` يديران التعاملات المالية والشراء العام. `%pay` ينشئ تحويلًا يدويًا بين المستخدمين، و`%give` ينشئ مطالبة يمكن استلامها. إذا أردت تحويل النص أو رسائل لوحة المفاتيح، استخدم `%convert`.

10. **الواجهة التفاعلية**
    أي زر تراه في البوت غالبًا تابع لنظام محدد داخل `customId`. أزرار البنك تبدأ بـ `bank:`، وأزرار المتجر بـ `shop:`، وأزرار التقدم بـ `progression:`، وأزرار التصنيع بـ `craft:`، وأزرار البيع بـ `sell:`. إذا فتح لك البوت نافذة منبثقة، فأنت عادةً أمام نموذج إدخال وليس أمرًا نصيًا، لذلك أكمل الحقول ثم أرسلها كما هي.

11. **تسلسل اللعب اليومي المقترح**
    الترتيب العملي الأفضل هو: افتح `%profile`، تأكد من HP والعتاد، ثم صيد الموارد بالأمر المناسب، وبعدها استخدم `%craft` لتحويل المواد إلى عناصر أو طقم، ثم `%sell` إذا أردت البيع، ثم `%bank` و`%shop` إذا كنت تحتاج إدارة مالية أو شراء مباشر. هذا التسلسل يقلل الخطأ ويمنع الهدر في HP أو المواد.

12. **متى تستخدم `%man`**
    إذا نسيت صيغة أمر معيّن، استخدم `%man <command>` بدل التخمين. هذا مفيد خاصة مع الأوامر التي لها سياق خاص مثل `%craft` و`%sell` و`%clan admincreate`. دليل الأمر يعرض لك الصيغة والأمثلة مباشرة من داخل البوت.

13. **فهم أنظمة الصلاحيات**
    ليس كل أمر متاحًا للجميع. الأوامر العامة تعمل مباشرة، بينما بعض أوامر التقدم والإدارة تتطلب صلاحية أو وضعًا خاصًا. إذا رفض لك البوت أمرًا، فغالبًا السبب هو المسار الحالي، أو عدم امتلاك الصلاحية، أو كون الأمر محجوبًا بواسطة الإدارة.

14. **قاعدة سريعة قبل التجربة**
    إذا كنت غير متأكد من قناة الاستخدام، أو المسار المطلوب، أو الرمز الصحيح، فارجع إلى `%profile` ثم `%help` ثم `%man <command>`. هذه الثلاثية تكفي غالبًا لمعرفة أين أنت، وما الذي يمكنك فعله الآن، وما الصيغة الدقيقة التالية.