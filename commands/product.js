...
async function addProductFromModal(interaction) {
    try {
        ...
        if (!name || !priceStr) {
            return interaction.reply({ content: '❌ Name and price are required. الاسم والسعر مطلوبان.', ephemeral: true });
        }

        const price = parseInt(priceStr, 10) * 10; // convert display gold → internal
        if (isNaN(price) || price <= 0) {
            return interaction.reply({ content: '❌ Price must be a positive number. السعر يجب أن يكون رقم موجب.', ephemeral: true });
        }
        ...
        return interaction.reply({ content: `✅ Product "${name}" added for ${price / 10} gold!`, ephemeral: true });
    } catch (err) {
        console.error('addProductFromModal error:', err);
        return interaction.reply({ content: '❌ Error adding product. خطأ في إضافة المنتج.', ephemeral: true });
    }
}

module.exports = { addProduct, addProductFromModal };